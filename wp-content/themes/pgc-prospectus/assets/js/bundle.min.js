...
( function( window ) {
	...
	h = {
		...
		extend( destination, source, deep, handleErrors ) {
			let sourceKeys = [],
				key = '',
				i = -1;
		...
		getCustomEvent( eventType, detail, doc ) {
			let event = null;
		...
		getHypotenuse( node1, node2 ) {
			let distanceX = node1.x - node2.x,
		...
		getIntersectionRatio( box1, box2 ) {
			let controlArea = box1.width * box1.height,
				intersectionX = -1,
				intersectionY = -1,
				intersectionArea = -1,
				ratio = -1;
		...
		closestParent( el, selector, includeSelf, doc ) {
		...
		compareVersions( control, specimen ) {
			let controlParts = control.split( '.' ),
		...
		getClassname( classNames, elementName, modifier ) {
			let classname = '';
	...
	mixitup.Base.prototype = {
		...
		callFilters( filterName, input, args ) {
			let self = this,
	...
	h.extend( mixitup.Control.prototype,
		/** @lends mixitup.Control */
		{
			...
			updateLive( command, toggleArray ) {
				let self = this,
					controlButtons = null,
					actions = null,
					button = null,
			...
			parseStatusChange( button, command, actions, toggleArray ) {
				let self = this,
					alias = '',
					toggle = '',
					i = -1;
	...
	h.extend( mixitup.Mixer.prototype,
		/** @lends mixitup.Mixer */
		{
			...
			attach( container, document, id, config ) {
				let self = this,
					target = null,
					i = -1;
			...
			buildToggleArray( command, state ) {
				let self = this,
			...
			splitCompoundSelector( compoundSelector ) {
				// Break at a `.` or `[`, capturing the delineator

				let partials = compoundSelector.split( /([\.\[])/g ),
					toggleArray = [],
					selector = '',
					i = -1;
			...
			insertTargets( command, operation ) {
				let self = this,
					nextSibling = null,
					insertionIndex = -1,
					frag = null,
			...
			getNextSibling( index, sibling, position ) {
				let self = this,
					element = null;
			...
			evaluateHideShow( testResult, target, action, operation ) {
				let self = this,
			...
			printSort( isResetting, operation ) {
				let self = this,
			...
			parseSortString( sortString, command ) {
				let self = this,
			...
			parseEffect( effectName, effectString, effects, transform, isOut ) {
				let self = this,
					re = /\(([^)]+)\)/,
					propIndex = -1,
					str = '',
			...
			insertDatasetFrag( frag, nextEl, targets ) {
				const self = this;
				let insertAt = nextEl ? h.arrayFromList( self.dom.parent.children ).indexOf( nextEl ) : self.targets.length;
			...
			willSort( sortCommandA, sortCommandB ) {
				let self = this,
					result = false;
	...
	h.extend( mixitup.Target.prototype, {
		...
		writeTransitionRule( property, staggerIndex, duration ) {
			let self = this,
