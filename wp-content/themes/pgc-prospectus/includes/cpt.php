...
add_action( 'init', 'register_cpt_campus' );


function register_cpt_campus() {
	// CPT Labels
	$cpt_singular_capital   = 'Campus'; // Name of the post type shown in the menu
	$cpt_plural_capital     = 'Campuses';
	$cpt_singular_lowercase = 'campus';
	$cpt_plural_lowercase   = 'campuses';

	// CPT Slug & Name
	$cpt_register_key = 'campus';  // This is the registering name of the single CPT post. (Try to keep it singular).
	$cpt_slug         = 'campus';  // This is the permalink slug of single CPT post. (Try to keep it singular).
	// The slug will become - www.website.com/campus/single-campus-name
	...
	$args   = array(
		...
		'has_archive'        => false, // Set this value for each CPT.
		'hierarchical'       => true, // Set this value for each CPT.
		'menu_icon'          => 'dashicons-admin-multisite', // Set this value for each CPT.
		'rewrite'            => array(
			'slug'       => $cpt_slug,
			'with_front' => true, // If required only then set this value for each CPT.
		),
